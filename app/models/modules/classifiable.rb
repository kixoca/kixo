module Classifiable
  def self.included(base)
    ...
    def base.find_all_by_category(category)
      self.joins(:categories).where(:conditions => {:professions => {:id => category}}).uniq
    end

    def base.find_all_by_topic(topic)
      self.joins(:topics).where(:conditions => {:professions => {:id => topic}}).uniq
    end

    def base.find_all_by_profession(profession)
      self.joins(:professions).where(:conditions => {:professions => {:id => profession}}).uniq
    end
